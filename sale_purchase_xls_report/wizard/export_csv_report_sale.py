...
class ExportCsvReportSale(models.TransientModel):
    _name= "export.csv.report.sale"
...
class SaleOrder(models.Model):
    ...
    def print_sale_order_xls(self):
        ...
        for selected_id in active_ids:
            ...
            col = 0
            state = ''
            selected_record = self.env['sale.order'].browse(selected_id)            
            if selected_record.state == 'draft' or selected_record.state == 'sent':
                ...
                title = 'Sale Order'
                worksheet_name = 'SO-' + selected_record.name[2:]
                if selected_record.state == 'cancel':
                    state = 'Cancelled' 
                elif selected_record.state == 'done':
                    state = 'Locked'
                elif selected_record.state == 'sale':
                    state = 'Sales Order' 
            ...
            worksheet.write(5, 3, 'Customer Ref.', left_bold)
            if selected_record.client_order_ref:
                worksheet.write_merge(5, 5, 4, 5, selected_record.client_order_ref)
            
            worksheet.write(6, 3, 'State', left_bold)
            worksheet.write_merge(6, 6, 4, 5, state)    
            
            #customer detail
            worksheet.write(row, col, 'Customer', left_bold)
            col += 1
            ...
            if selected_record.partner_id.phone:
                ...
                row+=1
            row+=1    
            worksheet.write(row, 0, 'Salesperson', left_bold)
            worksheet.write(row, 1, 'Source Document', left_bold)
            ...
            if selected_record.origin:
                worksheet.write(row, 1, selected_record.origin)
            if selected_record.incoterm:
                worksheet.write(row, 2, selected_record.incoterm.name)
            ...
            if selected_record.order_line:
                ...
                for order_line_record in selected_record.order_line:
                    ...
                    col+=1
                    if order_line_record.product_uom_qty:
                        worksheet.write(row,col, order_line_record.product_uom_qty, xlwt.easyxf('align:vertical center, horizontal center;'))
                    col+=1
                    ...
                    col+=1
                    if order_line_record.tax_id:
                        worksheet.write(row,col,', '.join(map(lambda x: x.name, order_line_record.tax_id)))
                    col+=1
        ...
        fp = StringIO()
        workbook.save(fp)
        export_obj = self.env['export.csv.report.sale'].create({'excel_file': base64.encodestring(fp.getvalue()), 'file_name': filename})
        fp.close()     
        return{
            'view_mode': 'form',
            'res_id': export_obj.id,
            'res_model': 'export.csv.report.sale',
            'view_type': 'form',
